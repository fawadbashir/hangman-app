...
class Hangman extends Component {
  ...
  */
  ...
  resetGame() {
    ...
    // this.props.maxWrong = word.length
  }
  gameStatus() {
  ...
  render() {
    ...
    return (
      <div className='Hangman'>
        ...
        {<p id = 'incorrect'>{`Wrong guesses ${this.state.nWrong <= this.props.maxWrong ? this.state.nWrong : this.props.maxWrong}`}</p>}
        <p className='Hangman-word'>{this.state.nWrong === this.props.maxWrong ? this.state.answer : this.guessedWord()}</p>
        <p className='Hangman-btns'>{this.gameStatus()}</p>
        {  !this.generateButtons() ?  '' : <button id = 'btn-reset' onClick = {this.resetGame}>Reset</button> }
      </div>
    );
  }
}
